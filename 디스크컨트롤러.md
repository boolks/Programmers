...
def solution(jobs):
    ...
    for job in jobs:
        heapq.heappush(heap_list, [job[1], job[0]])
    ...
    while heap_list:
        if not tmp:
            ...
            job = tmp.pop(0)
        print(heap_list, "job:", job, "시간", duration, "tmp", tmp)
        if job[1] <= duration:
            duration += job[0]
            answer += duration - job[1]
        else:
            if heap_list:
                for heap in heap_list:
                    if heap[1] <= duration:
                        tmp.append(heap)
                        heap_list.remove(heap)
...
답을 구하기 위해 heap를 사용했는데 heap의 기준을 소요시간으로 정렬하기 위해서 0번 째와 1번 째 인덱스의 순서를 바꿔주었다.